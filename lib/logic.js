...
// CME equity-index futures: Sun 6pm ET → Fri 5pm ET, daily maintenance break 5–6pm ET.
// "No overnight holding" = positions must be flat across the 5pm ET daily close, so we
// flatten + block new trades in the close/break window (and whenever the market is shut).
function noHoldAt(dow, hour, min) {
  const t = hour * 60 + min;
  if (dow === 6) return true;                          // Saturday — closed
  if (dow === 0 && t < 18 * 60) return true;           // Sunday before 6:00pm — closed
  if (dow === 5 && t >= 17 * 60) return true;          // Friday after 5:00pm — weekend
  if (t >= 16 * 60 + 59 && t < 18 * 60) return true;   // Mon–Fri 4:59–6:00pm close/break
  return false;
}
...
function enforceRisk(user, px) {
  ...
  // No overnight holding: flatten any open position across the daily session close.
  if (noHold() && hasOpen(user)) flattenAll(user, px, 'SESSION CLOSE — no overnight holding');

  const equity = user.balance + unrealized(user, px);
  if (equity > user.highEquity) user.highEquity = equity;

  if (!user.blown && equity <= user.highEquity - MAX_TRAILING_DRAWDOWN) {
    flattenAll(user, px, 'DRAWDOWN BREACH — account blown');
    user.blown = true;
  } else if (!user.blown && !user.dailyLocked && equity - user.dayStartEquity <= -DAILY_LOSS_LIMIT) {
    flattenAll(user, px, 'DAILY LOSS LIMIT — locked until next session');
...
function accountView(user, px) {
  ...
  return {
    ...
    drawdownFloor: round2(user.highEquity - MAX_TRAILING_DRAWDOWN),
    highEquity: round2(user.highEquity),
    blown: user.blown,
...
function placeOrder(user, px, symbol, side, qty) {
  ...
  enforceRisk(user, px);
  if (user.blown) return { error: 'Account blown (max drawdown breached). Competition over for this account.' };
  if (user.dailyLocked) return { error: 'Daily loss limit hit — trading locked until the next session.' };
  if (noHold()) return { error: 'No overnight holding — market is closed. Positions flatten at the 5:00pm ET close; trading resumes at 6:00pm ET (Sun–Fri).' };
...
module.exports = {
  SPECS, START_BALANCE, MAX_TRAILING_DRAWDOWN, DAILY_LOSS_LIMIT,
  ...
  flattenAll, newUser, leaderboard, hashPassword, safeEqual,
