...
// Cash indices on Yahoo are real-time & free (futures feeds are ~10min delayed).
const SPECS = {
  // maxContracts capped so full size = $40 per point on each ($10×4, $2×20).
  SP500:  { name: 'S&P 500',    pointValue: 10, tick: 0.1, maxContracts: 4,  feed: '^GSPC' },
  NAS100: { name: 'Nasdaq 100', pointValue: 2,  tick: 0.1, maxContracts: 20, feed: '^NDX' },
};
...
function placeOrder(user, px, symbol, side, qty) {
  ...
  if (!spec) return { error: 'Unknown symbol. Tradeable: SP500, NAS100.' };
...
function leaderboard(users, pxBySym) {
  return users
    ...
    .slice(0, 1000)
    .map(({ _c, _e, ...row }) => ({ ...row, email: _e })); // email included — leaderboard is admin-only
}
